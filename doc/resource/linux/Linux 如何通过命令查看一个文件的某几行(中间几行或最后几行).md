...
https://www.cnblogs.com/CoolJayson/p/7512814.html
...
o
u
self/\1r/ 标记元字符之间的模式，并将其保存为标签1，之后可以使用\1来引用它。最多可以定义9个标签，从左边开始编号，最左边的是第一个。此例中，对第1到第20行进行处理，you被保存为标签1，如果发现youself，则替换为your。
...
# linux 查看指定行数
```text
在Linux系统中，如果你想查看文件的特定行数，有几种方法可以实现。下面是一些常见的方法：

1. 使用sed命令

sed（stream editor）是一个非常强大的文本处理工具，可以用来查看文件的特定行。例如，如果你想查看第5行，可以使用：

sed -n '5p' filename

这里，-n选项用于禁止自动打印所有行，5p表示打印第5行。

2. 使用awk命令

awk是另一个强大的文本处理工具，它同样可以用来查看文件的特定行。例如，查看第5行：

awk 'NR==5' filename

或者更常见的：

awk 'NR==5 {print; exit}' filename

这里，NR==5表示当行号等于5时执行操作，print打印该行，exit退出，防止打印更多行。

3. 使用head和tail组合

如果你想要查看文件的第5行，可以先使用head命令获取前5行，然后使用tail命令获取最后1行：

head -n 5 filename | tail -n 1
4. 使用sed的范围功能

你也可以使用sed的范围功能来打印多行，例如打印第5到第10行：

sed -n '5,10p' filename
5. 使用tail和head组合查看多行（反向）

如果你想要查看从文件末尾开始的第5行到第10行（例如，查看文件的最后几行），可以先使用tail获取最后10行，然后使用head获取前5行：

tail -n 10 filename | head -n 5
6. 使用nl和管道（查看带有行号的文件）

如果你想要查看带有行号的文件中的特定行号（例如第5行），可以先使用nl命令为文件添加行号，然后使用其他命令如grep来过滤：

nl filename | grep '^ *5'

这里，^ *5匹配以空格后跟数字5开头的行。注意这里的空格是为了匹配可能的额外空格（如果行号前面有空格）。

以上方法可以根据你的具体需求选择使用。每种方法都有其适用场景，例如，如果你只是想快速查看一个文件的特定几行，使用sed, awk, head, tail的组合会更加直接和方便。如果你需要更复杂的文本处理，那么sed和awk会是更好的选择。
```